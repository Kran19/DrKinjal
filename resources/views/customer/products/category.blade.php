        ...
        <div class="flex flex-wrap justify-center gap-3 mb-16">
            ...
            @if(isset($filters['categories']) && count($filters['categories']) > 0)
                @foreach ($filters['categories'] as $cat)
                    <a href="{{ route('customer.category.products', ['slug' => $cat['slug']]) }}" 
                       class="category-tab px-5 py-2.5 bg-white text-slate-700 border-slate-200 font-medium rounded-full hover:shadow-lg transition-all border flex items-center
                              {{ request()->route('slug') == $cat['slug'] ? 'active ring-2 ring-brand-500 border-brand-500 text-brand-700' : '' }}">
                        <i data-lucide="sparkles" class="w-4 h-4 mr-2"></i> {{ $cat['name'] }}
                    </a>
                @endforeach
            @endif
        </div>
...
<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="flex flex-col gap-4 pb-10 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-2xl font-semibold tracking-tight text-gray-900" id="category-title">
            {{ isset($category) ? $category->name : 'All Products' }}
        </h2>
        <p class="text-gray-500" id="category-description">
            Showing {{ isset($paginator) ? $paginator['total'] : 0 }} products
        </p>
    </div>
        ...
        <div class="mt-16 flex justify-center">
            <div class="flex gap-2">
                ...
                @if ($paginator['current_page'] > 1)
                    <a href="{{ route('customer.category.products', array_merge(['slug' => request()->route('slug')], request()->query(), ['page' => $paginator['current_page'] - 1])) }}" 
                ...
                @for ($i = 1; $i <= $paginator['last_page']; $i++)
                    <a href="{{ route('customer.category.products', array_merge(['slug' => request()->route('slug')], request()->query(), ['page' => $i])) }}"
