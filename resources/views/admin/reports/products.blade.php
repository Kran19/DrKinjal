...
<script>
        ...
        productsReportTable = new Tabulator("#productsReportTable", {
            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter: function (cell) {
                        ...
                        const config = status === 'in_stock' 
                            ? { class: 'text-emerald-500', text: 'IN STOCK' }
                ...
                },
                {
                    ...
                    headerSort: false,
                    formatter: function (cell) {
                        return `<button onclick="viewReport(${cell.getValue()})" class="w-9 h-9 flex items-center justify-center bg-sky-50 text-sky-600 rounded-xl border border-sky-100 hover:bg-sky-100 transition-all shadow-sm"><i class="fas fa-chart-pie text-xs"></i></button>`;
                    }
                }
            ],
    ...
    function initColumnVisibility() {
        ...
        productsReportTable.getColumnDefinitions().forEach(column => {
            ...
            item.innerHTML = `<input type="checkbox" class="mr-3 rounded text-sky-500" ${column.visible !== false ? 'checked' : ''}> ${column.title}`;
