...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('admin.reviews.index') }}" class="inline-flex items-center text-stone-400 hover:text-sky-600 transition-colors mb-4 group font-bold text-sm uppercase tracking-widest">
            ...
            <form action="{{ route('admin.reviews.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    <div>
                        ...
                        <div class="relative">
                            <select name="product_id" id="product_id" class="w-full bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 text-stone-700 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all appearance-none cursor-pointer">
                    ...
                    <div>
                        ...
                        <input type="text" name="user_name" id="user_name" placeholder="e.g. Alexander Pierce"
                            class="w-full bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 text-stone-800 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-stone-300" 
                            value="{{ old('user_name') }}">
                        @error('user_name')
                            <p class="text-rose-500 text-xs font-bold mt-2 ml-1 tracking-tight">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center space-x-6 bg-stone-50 p-6 rounded-3xl border border-stone-100">
                        <div class="flex-1">
                            <input type="range" name="rating" id="rating_range" min="1" max="5" step="0.5" value="{{ old('rating', 5) }}"
                                class="w-full h-2 bg-stone-200 rounded-lg appearance-none cursor-pointer accent-sky-500">
                        ...
                        <div class="w-20 text-center">
                            <span id="rating_display" class="text-3xl font-black text-sky-600 tracking-tighter">{{ old('rating', 5.0) }}</span>
                ...
                <div class="mb-10">
                    ...
                    <textarea name="review" id="review" rows="6" placeholder="Write the customer's experience here..."
                        class="w-full bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 text-stone-800 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-stone-300 resize-none">{{ old('review') }}</textarea>
                    @error('review')
                        <p class="text-rose-500 text-xs font-bold mt-2 ml-1 tracking-tight">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pt-6 border-t border-stone-50">
                    <div class="flex items-center">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer group">
                            ...
                            <div class="w-11 h-6 bg-stone-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-stone-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sky-500 group-hover:scale-105 transition-transform"></div>
                            <span class="ml-4 text-xs font-black text-stone-400 uppercase tracking-widest peer-checked:text-sky-600 transition-colors">Visible to Public</span>
                        </label>
                    </div>
